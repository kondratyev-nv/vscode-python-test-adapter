...
export function parseTestSuites(
    ...
    const suites = Array.from(
        groupBy(allTests, t => t.modulePath).entries()
    ).map(
        ([modulePath, tests]) =>
            <TestSuiteInfo | TestInfo>{
                type: 'suite' as const,
    ...
    const aggregatedErrors = Array.from(
        groupBy(discoveryResult.errors || [], e => e.file).entries()
    ).map(([file, messages]) => ({
...
function toTestSuites(tests: ITestCaseSplit[]): (TestSuiteInfo | TestInfo)[] {
    ...
    const firstLevelTests: (TestSuiteInfo | TestInfo)[] = toFirstLevelTests(
        testsAndSuites.get(false)
    );
    const suites: (TestSuiteInfo | TestInfo)[] = toSuites(
        testsAndSuites.get(true)
    );
...
function toSuites(suites: ITestCaseSplit[] | undefined): TestSuiteInfo[] {
    ...
    return Array.from(
        groupBy(
            suites.map(test => splitTest(test)),
            group => group.idHead
        ).entries()
...
function toFirstLevelTests(
    tests: ITestCaseSplit[] | undefined
): (TestSuiteInfo | TestInfo)[] {
    if (!tests) {
        return [];
    }
    const testsByParameterized = groupBy(tests, t => t.idTail.includes('['));
    const basicTests: (TestSuiteInfo | TestInfo)[] = (
        testsByParameterized.get(false) || []
    ).map(toTest);
    const parameterizedTestsBySuite = groupBy(
        testsByParameterized.get(true) || [],
        t => t.idTail.substring(0, t.idTail.indexOf('['))
    );
    const parameterizedSuites: (TestSuiteInfo | TestInfo)[] = Array.from(
        parameterizedTestsBySuite.entries()
