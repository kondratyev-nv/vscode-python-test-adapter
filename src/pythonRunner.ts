...
export interface IPythonScriptRunConfiguration
    extends ICommonPythonRunConfiguration {
    script: string;
}
...
export function runModule(
    configuration: IPythonModuleRunConfiguration
): IProcessExecution {
    return run(
        ['-m', configuration.module].concat(configuration.args || []),
        configuration
    );
}

export function runScript(
    configuration: IPythonScriptRunConfiguration
): IProcessExecution {
    return run(
