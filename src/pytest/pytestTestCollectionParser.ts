...
export function parseTestSuites(content: string, cwd: string): (TestSuiteInfo | TestInfo)[] {
    ...
    const allTests = (discoveryResult.tests || [])
        ...
        .filter((line) => line)
        .map((line) => line!);
    const suites = Array.from(groupBy(allTests, (t) => t.modulePath).entries()).map(
        ([modulePath, tests]) =>
    ...
    const aggregatedErrors = Array.from(groupBy(discoveryResult.errors || [], (e) => e.file).entries()).map(
        ([file, messages]) => ({
...
function toFirstLevelTests(tests: ITestCaseSplit[] | undefined): (TestSuiteInfo | TestInfo)[] {
    ...
    const parameterizedTestsBySuite = groupBy(testsByParameterized.get(true) || [], (t) =>
        t.idTail.substring(0, t.idTail.indexOf('['))
    );
    const parameterizedSuites: (TestSuiteInfo | TestInfo)[] = Array.from(parameterizedTestsBySuite.entries()).map(
        ([baseName, parameterizedTests]) => ({
            type: 'suite' as 'suite',
