...
} from '../environmentVariablesLoader';
...
export class PytestTestRunner implements ITestRunner {
    ...
    public async debugConfiguration(
        ...
        const additionalEnvironment = await this.loadEnvironmentVariables(
            config
        );
        const runArguments = this.getRunArguments(
            test,
            config.getPytestConfiguration().pytestArguments
        );
        return {
            module: 'pytest',
            cwd: config.getCwd(),
    ...
    public async load(
        ...
        const discoveryArguments = this.getDiscoveryArguments(
            config.getPytestConfiguration().pytestArguments
        );
        this.logger.log(
            'info',
            `Running pytest with arguments: ${discoveryArguments.join(', ')}`
        );

        const result = await this.runPytest(
            config,
    ...
    public async run(
        ...
        const { file, cleanupCallback } = await this.getJunitReportPath(
            config.getCwd(),
            runArguments
        );
        ...
        this.logger.log(
            'info',
            `Running pytest with arguments: ${testRunArguments.join(', ')}`
        );

        const testExecution = this.runPytest(
            config,
            additionalEnvironment,
            testRunArguments
        );
    ...
    private async loadEnvironmentVariables(
        ...
        const envFileEnvironment = await EnvironmentVariablesLoader.load(
            config.envFile(),
            process.env,
            this.logger
        );
    ...
    private getDiscoveryArguments(rawPytestArguments: string[]): string[] {
        ...
        const [, argumentsToPass] =
            argumentParser.parse_known_args(rawPytestArguments);
        return ['--collect-only'].concat(argumentsToPass);
    }

    private getRunArguments(
        test: string,
        rawPytestArguments: string[]
    ): IRunArguments {
        const argumentParser = this.configureCommonArgumentParser();
        argumentParser.add_argument('--setuponly', '--setup-only', {
            action: 'store_true',
        });
        argumentParser.add_argument('--setupshow', '--setup-show', {
            action: 'store_true',
        });
        argumentParser.add_argument('--setupplan', '--setup-plan', {
            action: 'store_true',
        });
        argumentParser.add_argument('--collectonly', '--collect-only', {
            action: 'store_true',
        });
        argumentParser.add_argument('--trace', {
            dest: 'trace',
            action: 'store_true',
        });
        ...
        const [knownArguments, argumentsToPass] =
            argumentParser.parse_known_args(rawPytestArguments);
        return {
    ...
    private configureCommonArgumentParser() {
        ...
        argumentParser.add_argument('--rootdir', {
            action: 'store',
            dest: 'rootdir',
        });
        argumentParser.add_argument('-x', '--exitfirst', {
            dest: 'maxfail',
            action: 'store_const',
            const: 1,
        });
        argumentParser.add_argument('--maxfail', {
            dest: 'maxfail',
            action: 'store',
            default: 0,
        });
        ...
        argumentParser.add_argument('--lf', '--last-failed', {
            action: 'store_true',
            dest: 'lf',
        });
        argumentParser.add_argument('--ff', '--failed-first', {
            action: 'store_true',
            dest: 'failedfirst',
        });
        argumentParser.add_argument('--nf', '--new-first', {
            action: 'store_true',
            dest: 'newfirst',
        });
        argumentParser.add_argument('--cache-show', {
            action: 'store_true',
            dest: 'cacheshow',
        });
        ...
        argumentParser.add_argument('--pdb', {
            dest: 'usepdb',
            action: 'store_true',
        });
        argumentParser.add_argument('--pdbcls', { dest: 'usepdb_cls' });
        argumentParser.add_argument('--junitxml', '--junit-xml', {
            action: 'store',
            dest: 'xmlpath',
        });
        argumentParser.add_argument('--junitprefix', '--junit-prefix', {
            action: 'store',
        });
        return argumentParser;
    }

    private async createTemporaryFile(): Promise<{
        file: string;
        cleanupCallback: () => void;
    }> {
        return new Promise<{ file: string; cleanupCallback: () => void }>(
            (resolve, reject) => {
                tmp.file((error, file, _, cleanupCallback) => {
                    if (error) {
                        reject(
                            new Error(
                                `Can not create temporary file ${file}: ${error}`
                            )
                        );
                    }
