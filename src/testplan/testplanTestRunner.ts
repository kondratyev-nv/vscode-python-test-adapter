...
export class TestplanTestRunner implements ITestRunner {
    ...
    public async debugConfiguration(
        ...
        const runArguments = this.getRunArguments(
            test,
            config.getTestplanConfiguration().testplanArguments
        );
        return {
    ...
    public async load(
        ...
        this.logger.log(
            'info',
            `Running testplan with arguments: ${discoveryArguments.join(', ')}`
        );

        const result = await this.runTestPlan(
            config,
    ...
    public async run(
        ...
        const testRunArguments = [`--xml=${dirName}`].concat(
            runArguments.argumentsToPass
        );
        this.logger.log(
            'info',
            `Running testplan with arguments: ${testRunArguments.join(', ')}`
        );

        const testExecution = this.runTestPlan(
            config,
            additionalEnvironment,
            testRunArguments
        );
    ...
    private getDiscoveryArguments(rawTestplanArguments: string[]): string[] {
        ...
        const [, argumentsToPass] =
            argumentParser.parse_known_args(rawTestplanArguments);
        return ['--info', 'pattern-full'].concat(argumentsToPass);
    }

    private getRunArguments(
        test: string,
        rawTestplanArguments: string[]
    ): IRunArguments {
        const argumentParser = this.configureCommonArgumentParser();
    ...
    private configureCommonArgumentParser() {
        ...
        argumentParser.add_argument('--runpath', {
            action: 'store',
            dest: 'runpath',
        });
        argumentParser.add_argument('--stdout-style', {
            action: 'store',
            dest: 'stdout_style',
        });
        argumentParser.add_argument('--xml', {
            action: 'store',
            dest: 'xmlpath',
        });
    ...
    private async createTemporaryDirectory(): Promise<{
        dirName: string;
        cleanupCallback: () => void;
    }> {
        return new Promise<{ dirName: string; cleanupCallback: () => void }>(
            (resolve, reject) => {
                tmp.dir((error, dirName, cleanupCallback) => {
                    if (error) {
                        reject(
                            new Error(
                                `Can not create temporary directory ${dirName}: ${error}`
                            )
                        );
                    }
